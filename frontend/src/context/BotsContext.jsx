...
export function BotsProvider({ children }) {
  const [userId, setUserId] = useState(nullimport { createContext, useContext, useState, useEffect, useRef } from "react";
import { supabase } from "../supabaseClient";
...
export function BotsProvider({ children }) {
  ...
  const executeTrade = async (botType) => {
    ...
    try {
      // Win rate climbs the longer the bot has been running (minutes since first start),
      // capped so it doesn't run away.
      const minutesElapsed = bot.startedAt ? (Date.now() - bot.startedAt) / 60000 : 0;
      const timeBonus = Math.min(0.18, minutesElapsed * 0.012); // up to +18% over ~15 min
      const winRate = Math.min(0.88, 0.68 + timeBonus + Math.random() * 0.03);
      const isWin = Math.random() < winRate;

      // Wins: $0.75–$1.50 · Losses: capped so they never exceed $0.98
  ...
  const startBot = (botType) => {
    setBots((prev) => {
      ...
      if (!bot) return prev;
      // startedAt persists across stop/start so the win-rate time bonus keeps climbing
      const updated = { ...bot, active: true, startedAt: bot.startedAt || Date.now() };
